...
Esta guia explica como crear y crecer un servicio generado con `Sales`. Esta pensada para el dev que acaba de crear un proyecto y necesita saber donde va cada clase, que defaults ya vienen configurados, y cuando usar automations, handlers, hooks, jobs, consumers y exception handling.
...
Cada proyecto generado incluye `sales.json` en la raiz de la solucion:
...
Usa ese archivo para saber con que version de `Sales` se creo el servicio. En el codigo fuente del repositorio puede aparecer como `0.0.0-local`; los paquetes publicados se estampan durante el release con la version real del paquete.
  ...
  Sales.Business/
    ...
    Services/
      Audit/
        ...
        AuditEntry.cs
    Constants.cs
    Sales.Business.csproj
  Sales.Domain/
    Sales.Domain.csproj
  Sales.Persistence/
    AppDbContext.cs
    Sales.Persistence.csproj
tests/
  Sales.Tests/
    Testing/
      TemplateTestHost.cs
...
La mayor parte del wiring vive en `Sales.Api/DependencyInjection`.
...
namespace Sales.Business.Search.Boundaries.Transactions;
...
- [Sales NuGet](https://www.nuget.org/packages/Sales)
