...
This guide explains how to create and grow a service generated with `Sales`. It is written for the developer who just created a project and needs to know where code goes, which defaults are already wired, and when to use automations, handlers, hooks, jobs, consumers, and exception handling.
...
Every generated project includes `sales.json` at the solution root:
...
Use that file to know which `Sales` package version created the service. In the repository source it can show `0.0.0-local`; published template packages are stamped during release with the real package version.
...
Most application wiring lives in `Sales.Api/DependencyInjection`.
