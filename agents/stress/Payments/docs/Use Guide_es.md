...
Esta guia explica como crear y crecer un servicio generado con `Payments`. Esta pensada para el dev que acaba de crear un proyecto y necesita saber donde va cada clase, que defaults ya vienen configurados, y cuando usar automations, handlers, hooks, jobs, consumers y exception handling.
...
Cada proyecto generado incluye `payments.json` en la raiz de la solucion:
...
Usa ese archivo para saber con que version de `Payments` se creo el servicio. En el codigo fuente del repositorio puede aparecer como `0.0.0-local`; los paquetes publicados se estampan durante el release con la version real del paquete.
...
La mayor parte del wiring vive en `Payments.Api/DependencyInjection`.
