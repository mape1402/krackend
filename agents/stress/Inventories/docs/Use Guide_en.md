...
This guide explains how to create and grow a service generated with `Inventories`. It is written for the developer who just created a project and needs to know where code goes, which defaults are already wired, and when to use automations, handlers, hooks, jobs, consumers, and exception handling.
...
Every generated project includes `inventories.json` at the solution root:
...
Use that file to know which `Inventories` package version created the service. In the repository source it can show `0.0.0-local`; published template packages are stamped during release with the real package version.
...
Most application wiring lives in `Inventories.Api/DependencyInjection`.
