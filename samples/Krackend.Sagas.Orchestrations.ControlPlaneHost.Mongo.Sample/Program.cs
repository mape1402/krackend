...
using Krackend.Sagas.Orchestrations.ControlPlaneHost.Mongo.Sample.Bootstrap;
...
if (string.IsNullOrWhiteSpace(mongoConnection))
    throw new InvalidOperationException("Set ConnectionStrings:Mongo for the Design Mongo sample host.");

var redisConnection = builder.Configuration.GetConnectionString("Redis");
var mongoDatabaseName = builder.Configuration.GetValue("Mongo:DatabaseName", "krackend-control-plane");
...
builder.Services.AddOrchestratorControlPlane(options =>
{
    ...
    options.ConfigureStorage = db => db.UseMongoDB(mongoConnection, mongoDatabaseName);
});
