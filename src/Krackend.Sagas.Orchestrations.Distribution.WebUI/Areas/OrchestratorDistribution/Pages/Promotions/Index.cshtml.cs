...
namespace Krackend.Sagas.Orchestrations.Distribution.WebUI.Areas.OrchestratorDistribution.Pages.Releases;

public sealed class IndexModel : PageModel
{
    private readonly IReleaseInteractionService _promotionService;
    private readonly IReleaseTargetInteractionService _releaseTargetService;
    private readonly IArtifactInteractionService _artifactService;
    private readonly IRuntimeNodeInteractionService _runtimeService;
    ...
    public IndexModel(
        IReleaseInteractionService promotionService,
        IReleaseTargetInteractionService releaseTargetService,
        IArtifactInteractionService artifactService,
        IRuntimeNodeInteractionService runtimeService,
    ...
    public async Task<IActionResult> OnPostCreateAsync(CancellationToken cancellationToken = default)
    {
        ...
        var selectedArtifact = (await _artifactService.GetAll(new InteractionPagedSettings { PageNumber = 1, PageSize = 400 }, cancellationToken))
            .Rows
    ...
    {
        var allRows = (await _promotionService.GetAll(new InteractionPagedSettings { PageNumber = 1, PageSize = 100 }, cancellationToken)).Rows;
        Orchestrations = await _policyService.GetOrchestrations(cancellationToken);
        Artifacts = (await _artifactService.GetAll(new InteractionPagedSettings { PageNumber = 1, PageSize = 300 }, cancellationToken)).Rows;
        if (string.IsNullOrWhiteSpace(OrchestrationId))
        {
        ...
        RuntimeNodes = (await _runtimeService.GetAll(new InteractionPagedSettings { PageNumber = 1, PageSize = 200 }, cancellationToken)).Rows.Where(x => x.IsEnabled).ToArray();
        var releaseIds = Rows.Select(x => x.Id).ToHashSet(StringComparer.Ordinal);
        var releaseTargets = (await _releaseTargetService.GetAll(new InteractionPagedSettings { PageNumber = 1, PageSize = 1000 }, cancellationToken)).Rows
