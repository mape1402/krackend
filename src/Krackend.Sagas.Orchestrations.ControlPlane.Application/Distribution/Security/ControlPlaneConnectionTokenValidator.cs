...
/// <summary>
/// Default Design bearer token validator.
/// </summary>
public sealed class ControlPlaneConnectionTokenValidator : IControlPlaneConnectionTokenValidator
{
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="ControlPlaneConnectionTokenValidator"/> class.
    /// </summary>
    public ControlPlaneConnectionTokenValidator(
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var validationKey = _cacheKeyBuilder.BuildValidationKey("design", tokenHash);
        ...
        if (!string.IsNullOrWhiteSpace(cachedValidation))
        {
            ...
            if (cachedPrincipal is not null &&
                string.Equals(cachedPrincipal.NodeId, runtimeNodeId, StringComparison.Ordinal) &&
        ...
        if (!string.Equals(entry.NodeId, runtimeNodeId, StringComparison.Ordinal))
        {
            return ConnectionTokenValidationResult.Failure("Bearer token does not belong to the requested runtime node.");
        }

        var node = await _repository.GetById(new Id(Ulid.Parse(entry.NodeId)), cancellationToken);
        if (node is null || node.IsDeleted || node.Status != RuntimeNodeStatus.Enabled || node.InboundCredentialStatus != ConnectionCredentialStatus.Active)
        {
