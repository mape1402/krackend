...
public sealed class RuntimeBackChannelResponseHandlerTests
{
    ...
    public async Task Handle_Should_Schedule_Response_Ingress_With_Trace_Metadata()
    {
        ...
        Assert.Equal(store.DispatchId.ToString(), envelope.DispatchId);
        Assert.Equal(store.TaskId.ToString(), envelope.TaskExecutionId);
        ...
        Assert.Equal(store.DispatchId.ToString(), envelope.Source.MessageId);
    ...
    [Fact]
    public async Task Handle_Should_Defer_Trace_Validation_To_Durable_Ingress()
    {
        var scheduler = new CapturingDurableWorkScheduler();
        var store = BackChannelStore.Create();
        var handler = new RuntimeBackChannelResponseHandler(scheduler);
        var crossedInstanceId = Id.New().ToString();
        var context = CreateContext(store, instanceId: crossedInstanceId);

        await handler.Handle(context);

        var envelope = Assert.IsType<RuntimeIngressEnvelope>(scheduler.ProcessIngressEnvelope);
        Assert.Equal(crossedInstanceId, envelope.OrchestrationInstanceId);
    }
    ...
    private static MessageConsumeContext CreateContext(
        ...
        => new()
        {
            ...
            Metadata = new OrchestratorMessageMetadata
            {
                ...
                TaskExecutionId = taskExecutionId ?? store.TaskId.ToString(),
                DispatchId = dispatchId ?? store.DispatchId.ToString(),
